...
def get_parser():
    parser = argparse.ArgumentParser(
            description='The script to run object detection on one image or one video')
    parser.add_argument(
            '--model_path',
    ...
    parser.add_argument(
            '--input_path',
            type=str, action='store',
            help='Path to the input file that contains the image/video to apply YOLO on')
    parser.add_argument(
            '--output_path',
            type=str, action='store',
            help='Path to the output file that saves the detection result')
    parser.add_argument(
            '--obj_thresh',
            type=float, action='store',
            default=0.4,
            help='The objectness threshold')
    parser.add_argument(
            '--nms_thresh',
            type=float, action='store',
            default=0.45,
            help='The IoU threshold for non max suppression')
    parser.add_argument(
...
def detect_image(image_pil, darknet, obj_thresh=0.4, nms_thresh=0.45, net_h=416, net_w=416, anchors=anchors, labels=labels):
...
def detect_video(video_path, output_path, darknet, obj_thresh=0.4, nms_thresh=0.45, net_h=416, net_w=416, anchors=anchors, labels=labels):
    ...
    while vid.isOpened():
      ...
      if ret:
          ...
          result = detect_image(frame, darknet, obj_thresh=obj_thresh, nms_thresh=nms_thresh)
...
def main():
    ...
    # Load model
    darknet = tf.keras.models.load_model(args.model_path)

    if args.video:
        detect_video(args.input_path, args.output_path, darknet, obj_thresh=args.obj_thresh, nms_thresh=args.nms_thresh)        
    else:
        image_pil = Image.open(args.input_path)
        image_detect = detect_image(image_pil, darknet, obj_thresh=args.obj_thresh, nms_thresh=args.nms_thresh)
        image_detect.save(args.output_path)
